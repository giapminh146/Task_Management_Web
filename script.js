document.querySelectorAll('.navbar a').forEach(item => {
    item.addEventListener('click', event => {
        // Get the data-target of the clicked item
        const target = event.currentTarget.getAttribute('data-target');

        // Update the content depend on what u choose
        ...
        switch (target) {
            ...
            case 'pending':
                ...
                break;
            case 'in-progress':
                title.innerText = 'In Progress Tasks';
                description.innerText = 'These are the tasks currently in progress.';
                break;
            case 'deployed':
